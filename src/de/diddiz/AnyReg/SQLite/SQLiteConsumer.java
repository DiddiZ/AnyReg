...
public class SQLiteConsumer extends Consumer
{
	...
	public void run() {
		...
		try {
			...
			conn.commit();
			ps = conn.prepareStatement("INSERT INTO `ar-blacklisted` (type, x, y, z, worldid) SELECT ?, ?, ?, ? , rowid FROM `ar-worlds` WHERE worldname = ?");
